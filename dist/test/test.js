...
function testAliasImports() {
    ...
    const results = (0, index_1.parseImports)(additionalSpecialCases, config);
    console.log("\n=== Imports Valides ===");
    ...
    const outputPath = path_1.default.resolve(__dirname, `./results/test-special-cases-${timestamp}.json`);
...
function testDuplicateCorrection() {
    ...
    const timestamp = Date.now();
    const results = (0, index_1.parseImports)(duplicatesTestCases, config);
    ...
    // Écrire les résultats dans un fichier
    const outputPath = path_1.default.resolve(__dirname, `./results/test-duplicate-correction-${timestamp}.json`);
    (0, fs_1.writeFileSync)(outputPath, JSON.stringify(results, null, 2));
...
function testErrorCases() {
    ...
    // Exécuter les tests
    let passedTests = 0;
    const totalTests = errorCases.length;
    for (const testCase of errorCases) {
        console.log(`\nTest: ${testCase.name}`);
        const result = (0, index_1.parseImports)(testCase.code, config);
 ...
 */
function checkOutput(testCase, result) {
...
function testProblematicImports() {
    ...
    const results = (0, index_1.parseImports)(problematicImports, config);
    console.log("\n=== Imports Valides ===");
    results.groups.forEach((group) => {
        console.log(`\nGroupe: ${group.name} (${group.imports.length} imports)`);
        group.imports.forEach((imp, idx) => {
    ...
    // Écrire les résultats dans un fichier
    const timestamp = Date.now();
    const outputPath = path_1.default.resolve(__dirname, `./results/test-problematic-imports-${timestamp}.json`);
    (0, fs_1.writeFileSync)(outputPath, JSON.stringify(results, null, 2));
...
function testExtraCases() {
    ...
    const results = (0, index_1.parseImports)(extraTestCases, config);
    console.log("\n=== Imports Valides ===");
    results.groups.forEach((group) => {
        console.log(`\nGroupe: ${group.name} (${group.imports.length} imports)`);
        group.imports.forEach((imp, idx) => {
    ...
    // Écrire les résultats dans un fichier
    const timestamp = Date.now();
    const outputPath = path_1.default.resolve(__dirname, `./results/test-extra-cases-${timestamp}.json`);
    (0, fs_1.writeFileSync)(outputPath, JSON.stringify(results, null, 2));
