...
exports.applyMergedImports = applyMergedImports;
exports.processCodeAndMergeImports = processCodeAndMergeImports;
 ...
 */
function parseImportSpecifiers(raw, importType) {
    const specifiers = [];
 ...
 */
function mergeSpecifiers(target, newSpecifiers) {
 ...
 */
function generateImportStatement(source, specifiers, importType = 'named', config = {}) {
 ...
 */
async function mergeImports(code, config = {}) {
    ...
    const [imports] = await (0, es_module_lexer_1.parse)(code);
    const merged = new Map();
    for (const i of imports) {
        if (!i.n)
            continue; // Ignorer les imports sans module source
        const raw = code.slice(i.ss, i.se);
        const importType = raw.includes('import type')
 ...
 */
function applyMergedImports(code, mergedImports) {
 ...
 */
async function processCodeAndMergeImports(code, config = {}) {
    try {
