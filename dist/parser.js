...
class ImportParser {
    ...
    async findImportRange(sourceText) {
        try {
            const [imports] = await (0, es_module_lexer_1.parse)(sourceText);
            if (imports.length === 0) {
            ...
            for (const imp of imports) {
                if (imp.ss < firstImportStart)
                    firstImportStart = imp.ss;
                if (imp.se > lastImportEnd)
                    lastImportEnd = imp.se;
            }
            ...
            while (pos > 0) {
                ...
                if (commentStart === -1 || commentStart < 0)
                    break;
                pos = commentStart - 1;
            }
            return {
    ...
    detectGroupComments(sourceCode) {
        const foundGroups = [];
    ...
    async parse(sourceCode) {
        ...
        try {
            ...
            const ast = (0, parser_1.parse)(importSection, {
                sourceType: "module",
                plugins: ["typescript"],
                errorRecovery: true,
            });
    ...
    generateStandardFormattedCode(parse) {
        let result = "";
        for (const group of parse.groups) {
            result += `// ${group.name}\n`;
            for (const importObj of group.imports) {
