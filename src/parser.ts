...
import { mergeImports } from "./merger";
...
class ImportParser {
   ...
   */
  private async enhancedMergeImports(parsedImports: ParsedImport[], sourceCode: string): Promise<ParsedImport[]> {
    try {
      ...
      const enhancedMerged = await mergeImports(sourceCode, {
